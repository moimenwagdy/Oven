...
class CartNotifier extends _$CartNotifier {
  ...
  Future<void> addItem(dynamic payload) async {
    state = await AsyncValue.guard(() async {
      final cart = List<CartItem>.from(state.value ?? []);

      // Case 1 → payload is String (means "increase only")
      if (payload is String) {
        final index = cart.indexWhere((e) => e.id == payload);
        if (index != -1) {
          cart[index].quantity++;
        }
      }
      // Case 2 → payload is CartItem (means "add new or increase existing")
      else if (payload is CartItem) {
